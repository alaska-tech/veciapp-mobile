...
export default function ChatsScreen() {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      />
      <Card className="mx-4 mt-4 mb-2 shadow">
      ...
      <FlatList
        ref={flatListRef}
        data={[...messages]}
        ListEmptyComponent={messages.length===0?<Text>Error</Text>:null}
        keyExtractor={(item: any) => item.id|| '1'}
        renderItem={({ item:msg, index }: any) => {
          if (!msg)return <></>
          return (
          <View
            key={msg.id}
            className={`my-1 mx-4 max-w-[80%] ${
              msg.sender === "customer"
                ? "self-end ml-auto"
                : "self-start mr-auto"
            }`}
      ...
      />
      <View className="p-4 flex-row items-center pb-10">
        <TextInput
        ...
        <TouchableOpacity
          ...
          className={`w-12 h-12 rounded-full items-center justify-center ${
            message.trim() === "" ? "bg-green-300" : "bg-green-600"
          }`}

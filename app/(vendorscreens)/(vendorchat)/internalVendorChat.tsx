...
export default function InternalVendorChat() {
      ...
      <View className="flex-1">
        ...
        >
          {messages.map((msg) => (
            <View 
              key={msg.id} 
              className={`my-1 max-w-[80%] ${msg.sender === 'vendor' ? 'self-end ml-auto' : 'self-start mr-auto'}`}
            >
              <View 
              ...
              >
                <Text 
                  className={`${msg.sender === 'vendor' ? 'text-primary-foreground' : 'text-foreground'}`}
                >
                  {msg.text}
                </Text>
              </View>
